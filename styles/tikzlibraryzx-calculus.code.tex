...
\definecolor{colorZxZ}{named}{white}
\definecolor{colorZxX}{named}{lightgray}
\definecolor{colorZxH}{named}{white}
\definecolor{colorZxMatrix}{named}{white}
      ...
      \pgfintersectionofpaths{%
        ...
        \begin{pgfinterruptboundingbox}%
          ...
          \else%
            ...
            \ifnum#5=1\relax % <-- important space!!!
...
\NewDocumentEnvironment{ZXNoExt}{O{}}{%
  ...
  % quantikz patches tikzcd globally and injects \wire{a} in empty matrix cells.
  % This creates an unintended center wire in ZX diagrams (e.g. \leftManyDots ... \zxX{}).
  % We locally neutralize \wire in ZX diagrams to keep ZX wiring semantics explicit.
  \ifdefined\fullwire%
    \def\wire##1{}%
  \fi%
  \begin{tikzcd}[%
...
\zxNewNodeFromPic{Matrix}
  ...
  \node[zx main node, draw, signal, fill=colorZxMatrix, inner sep=1pt, minimum height=6pt, transform shape,
    ...
    label/.expanded={[
      ...
      % as if it gets expanded further, we can get errors (e.g.\ with bmatrix)
...
\NewExpandableDocumentCommand{\leftManyHDots}{O{1}O{\zxScaleDots}m}{%
  \node[zx main node, zxNone+,inner xsep=0pt]{%
    \scalebox{#1}{$#3$\,}%
    \makebox[0pt][l]{\scalebox{#2}{$\cvdotsCenterMathline$}}%
  };
  \ar[r,-N.,H,start anchor=north east]
  \ar[r,-N',H,start anchor=south east]
  \pgfmatrixnextcell[\zxwCol]%
}
...
\NewExpandableDocumentCommand{\rightManyHDots}{O{1}O{\zxScaleDots}m}{%
  \ar[r,N'-,H,end anchor=north west] \ar[r,N.-,H,end anchor=south west] \pgfmatrixnextcell[\zxwCol] \node[zx main node, zxNone+,inner xsep=0pt]{\makebox[0pt][r]{\scalebox{#2}{$\cvdotsCenterMathline$}}\scalebox{#1}{\,$#3$}};
}
