...
\begin{fact}<2->
$U \in SU(2^n)$ if and only if $U = e^{iH}$ for some $H \in \mathfrak{su}(2^n)$.
\end{fact}
...
\begin{fact}<3->
$H \in \mathfrak{su}(2^n)$ if and only if $H = -i \sum_{P \in \mathcal{P} \setminus \{I\}} \alpha_P P$ for some $\alpha_P \in \mathbb{R}$, where $\mathcal{P}$ is the set of Pauli matrices on $n$ qubits.
\end{fact}
...
\begin{fact}<2->
We cannot generate all $|\{I,X,Y,Z\}|^3 = 64$ Pauli operators on $3$ qubits this way.
\end{fact}
