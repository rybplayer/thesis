...
% Dirac notation: $\ket{0} = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$, $\ket{1} = \begin{pmatrix} 0 \\ 1 \end{pmatrix}$.
...
\lstick{$\ket{\psi_1}$} & \gate[3]{C} & \meter{} \\
\lstick{$\vdots$}       &             & \vdots   \\
\lstick{$\ket{\psi_n}$} &             & \meter{}
\end{quantikz}
};
...
\end{align*}
\only<2->{%
If we measure $0$, we get $HR_z(\theta)\ket\phi$ in the second register. If we measure $1$, we get $XH_z(\theta)\ket\phi$. Thus given the right measurements we can apply any 1-qubit gate.
}
\end{proof}
