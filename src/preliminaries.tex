...
\input{figures/single_qubit_gates.tex}
...
\input{figures/multi_qubit_gates.tex}
...
\note{This section is meant to introduce the ZX-calculus, a graphical representation of quantum circuits that makes MBQC in particular far easier to analyze. It would be most helpful for proofs in later sections as well. That said, it requires significant diagramming which I have unfortunately not done (and am still quite slow at). I am considering referring readers to the excellent ``ZX-calclus for the working computer scientist'' by J. van de Wetering \cite{zx-working}. If the preliminary section survives the rough draft, then I will give a more formal introduction of the ZX-calculus here with a focus on MBQC and its use in finding graph identities.

In particular, I would like to introduce the following ZX-rules: (the following figure is a screenshot from \cite{review-zx})}

\begin{figure}[H]
    \includegraphics[width=\linewidth]{media/ZX-Rules.png}
    \label{fig:zx-rules}
    \caption{The rules of ZX-calculus, taken from \cite{review-zx}}
\end{figure}
