...
\begin{definition} A single \term{qubit} $\ket\phi$ is a state
\begin{align}
    \ket\phi = \alpha\vect{1}{0} + \beta\vect{0}{1} \in \C^2,
\end{align}
where $|\alpha|^2 + |\beta|^2 = 1$ and $\alpha, \beta \in \C$. We say $\ket\phi$ is in a \term{superposition} of $\svect{1}{0}$ and $\svect{0}{1}$. Thus a single qubit is a length 2 vector in the Hilbert space $\C^2$ with $\ell_2$ norm 1.
\end{definition}
...
\end{align}
Now the $\ell_2$ norm of $\ket{\phi}$ can be written as $|\ket{\phi}| := \|\ket\phi\|_2 =  \sqrt{\braket{\phi}{\phi}}$.
\end{definition}
...
\begin{align}
    ...
    \end{bmatrix}.
\end{align}
where \(a_{i,j}\) denotes the entry of \(A\) in row \(i\) and column \(j\).
\end{definition}
...
\end{align}
For example, $\ket{00} = \ket{0} \otimes \ket{0} = \begin{bmatrix} 1 & 0 & 0 & 0 \end{bmatrix}^T$. We may also replace $x$ by its integer evaluation as a binary number, by abuse of notation. For example,
\begin{align}
    \ket{3} = \ket{11} = \begin{bmatrix}
        0 & 0 & 0 & 1
    \end{bmatrix}^T
\end{align}
but we may also have
\begin{align}
    \ket{3} = \ket{011} = \begin{bmatrix}
        0 & 0 & 0 & 1 & 0 & 0 & 0 & 0
    \end{bmatrix}^T
\end{align}
...
\begin{align}
    ...
    = \sum_{x=0}^{2^q - 1} \alpha_x \ket{x} 
    \in \C^{2^q}
\end{align}
where $\alpha_x \in \C$ and $\sum_{x \in \{0,1\}^d} |\alpha_x|^2 = 1$. As the number of dimensions increases exponentially with respect to the number of qubits, braket notation will become increasingly concise compared to traditional vector notation.
\end{definition}
...
\begin{enumerate}
    \item $\|\ket{v_i}\|_2 = 1$ for every $i \in [n]$ and
...
The \term{Born Rule} gives outcome probabilities for quantum measurements: An $n$-qubit state $\ket\phi$ can be measured under any orthonormal basis $\{\ket{v_i}\}_{i \in [2^n]}$ of $\C^{2^n}$. When measured, the outcome $i$ occurs with probability $| \braket{v_i}{\phi} |^2$. After seeing outcome $i$, the state collapses to $\ket{v_i}$. This process is irreversible.
\end{definition}
...
\begin{align}
    ...
    P_i^2 = P_i.
\end{align}
Define the \term{partial measurement} projector on $S_1$ by $M_i = P_i \otimes I \in \C^{2^n \times 2^n}$. For a state $\ket\phi \in \C^{2^n}$, 
\begin{align}
    \Pr(\text{Outcome } i \mid \ket\phi)
    = \bra\phi M_i\ket\phi 
    = \| M_i \ket\phi \|^2.
\end{align}
Conditioned on outcome $i$, the post-measurement state becomes
\begin{align}
    \ket{\phi_i}
...
\begin{definition}
A two-qubit state $\ket\phi$ is \term{entangled} if it cannot be written as the tensor product of single qubits. That is, there does not exist single-qubit states $\ket\psi, \ket\varphi \in \C^2$ such that $\ket\phi = \ket\psi \otimes \ket\varphi$. If a state is not entangled, it is called \term{separable}. A $n$-qubit state $\ket{\psi}$ is entangled (or \emph{contains} entanglement) if it is not fully separable.
\end{definition}
