...
The backwards direction follows from Lemma \ref{lemma:zx-local-complement-without-deletion}. The forward direction surmounts to showing that the successive application of local complementation generates the orbit of any graph state under local Clifford operations. Informally, the proof in \cite{stab-lc-equiv-graph} does this by a clever characterization of local Clifford operations as graph operations, before showing that all these operations can be implemented using local complementation alone.
\end{proof}
...
\begin{theorem}
A family of states $\Phi$ is a universal resource for MBQC if for all $n$-qubit states $\ket\psi$, $\exists \ket\phi \in \Phi$ with $m \geq n$ qubits, such that the transformation $\ket\phi \to \ket\psi$ is possible deterministically with $\LO$ and $\CC$. We denote this by $\ket\phi \geq_{\LOCC} \ket\psi$. 
\end{theorem}
...
\begin{align}
    ...
    \end{array}.
\end{align}
Hence the cut-rank of $A$ is $2$. Now if $u$ and $v$ are not adjacent, by symmetry is suffices to consider $A = \{1,3\}$. Then 
\begin{align}
    M_A = \begin{array}{c|c c c}
        & 2 & 4 & 5 \\
        \hline 
...
since local complementation does not change edges incident to $v$. This is an elementary row operation.
...
\[
    ...
    \end{cases}
\]
The column indexed by $v$ is unchanged. Once again, this is an elemetary column operation.
...
\begin{proof}
We first show that every complete graph is local-complementable to a star. Let $n$ be the number of vertices. $n=1$ is trivial. For $n \geq 2$, local complementation at any $v \in V(K_n)$ toggles all edges not incident to $v$. The result is a star rooted at $v$. By Theorem \ref{thm:local-complementation-preserves-rank-width} and Theorem \ref{thm:star-not-universal}, $\CompleteGraphs$ is not universal.
\end{proof}