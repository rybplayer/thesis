\subsection{The Stabilizer Formalism}

When we view the Clifford circuits of the previous section more algebraically, we find interesting connections to the following objects:

\begin{definition}
The \term{Pauli matrices} or \term{Pauli gates} refer to the following gates:
\begin{equation}
\begin{alignedat}{3}
    X = \begin{bmatrix}
        0 & 1 \\ 1 & 0
    \end{bmatrix}, \quad
    &Y = \begin{bmatrix}
        0 & -i \\ i & 0
    \end{bmatrix}, \quad
    &Z = \begin{bmatrix}
        1 & 0 \\ 0 & -1
    \end{bmatrix}.
\end{alignedat}
\end{equation}
\end{definition}

\begin{definition}
Say a matrix $A$ is a \term{stabilizer} of the state $\ket{\psi}$ when $\ket\psi$ is a +1 eigenvector of $A$, i.e. $A \ket{\psi} = \ket{\psi}$.
\end{definition}

\begin{definition}
\label{def:pauli-group}
The $n$-qubit Pauli group is defined as follows:
\begin{align}
    \cP_n := \{i^k P_1 \otimes \cdots P_n \mid 
    k \in \{0,1,2,3\}, P_j \in \{I,X,Y,Z\} \}.
\end{align}
Call the elements of $\cP_n$ as \term{Pauli strings}, written as vectors of length $n$:
\begin{align}
    \vec{P} := i^k P_1 \otimes \cdots \otimes P_n.
\end{align}
\end{definition}

\begin{proof}
We need to show that this is indeed a group. Associativity follows from matrix multiplication. $I$ is the identity. Each Pauli matrix is its own inverse, i.e. $X^2 = Y^2 = Z^2 = 1$, and it is easy to see that the inverses of $i^kP$ exist for all $k$ and $P$. Finally, since two non-identity Pauli matrices $P_1 \neq P_2$ anti-commute, i.e. $P_1 P_2 = -P_2 P_1$, an exhaustive multiplication table shows the group is closed and finite.
\end{proof}

\begin{theorem}
\label{thm:pauli-commute-clifford}
For any Clifford unitary $U$ and Pauli string $\vec{P}$ of compatible dimensions, there exists a pauli string $Q$ such that $U \vec{P} U^\dagger = \vec{Q}$.
\end{theorem}

\begin{proof}
It suffices to check that, when we commute a basis of the Pauli strings through the Hadamard, S, and CNOT gate, we will end up with another Pauli string. Notice that $\{X,Z\}$ is a basis for $\cP_1$, up to scalar factors. Now note that 
\begin{equation}
    \begin{ZX}
        \rar & \zxZ{\pi} \rar & \zxH{} \rar &
    \end{ZX} \overset{\zxref{cc}}{=} 
    \begin{ZX}
        \rar & \zxH{} \rar & \zxX{\pi} \rar &
    \end{ZX}, \quad 
    \begin{ZX}
        \rar & \zxX{\pi} \rar & \zxH{} \rar &
    \end{ZX} \overset{\zxref{cc}}{=} 
    \begin{ZX}
        \rar & \zxH{} \rar & \zxZ{\pi} \rar &
    \end{ZX}.
\end{equation}
For the S gate,
\begin{equation}
    \begin{ZX}
        \rar & \zxZ{\pi} \rar & \zxZ{\frac{\pi}{2}} \rar &
    \end{ZX} \overset{\zxref{f}}{=} 
    \begin{ZX}
        \rar & \zxZ{\frac{\pi}{2}}  \rar & \zxZ{\pi} \rar &
    \end{ZX}, \quad 
    \begin{ZX}
        \rar & \zxX{\pi} \rar & \zxZ{\frac{\pi}{2}}  \rar &
    \end{ZX} \overset{\substack{\zxref{\pi}\\\zxref{f}}}{=} 
    \begin{ZX}
        \rar & \zxZ{\frac{\pi}{2}}  \rar & \zxZ{\pi} \rar & \zxX{\pi} \rar &
    \end{ZX}.
\end{equation}
For the CNOT gate,
\begin{align}
    \begin{ZX}
        \rar & \zxZ{\pi} \rar & \zxZ{} \dar \rar & \\
        \rar & \rar           & \zxX{}      \rar &
    \end{ZX} \overset{\zxref{f}}{=} 
    \begin{ZX}
        \rar & \zxZ{} \dar \rar & \zxZ{\pi} \rar & \\
        \rar & \zxX{}      \rar &           \rar &
    \end{ZX}, \quad 
    \begin{ZX}
        \rar & \zxX{\pi} \rar & \zxZ{} \dar \rar & \\
        \rar & \rar           & \zxX{}      \rar &
    \end{ZX} \overset{\substack{\zxref{\pi}\\\zxref{f}}}{=} 
    \begin{ZX}
        \rar & \zxZ{} \dar \rar & \zxX{\pi} \rar & \\
        \rar & \zxX{}      \rar & \zxX{\pi} \rar &
    \end{ZX}, \\
    \begin{ZX}
        \rar &           \rar & \zxZ{} \dar \rar & \\
        \rar & \zxZ{\pi} \rar & \zxX{}      \rar &
    \end{ZX} \overset{\substack{\zxref{\pi}\\\zxref{f}}}{=} 
    \begin{ZX}
        \rar & \zxZ{} \dar \rar & \zxZ{\pi} \rar & \\
        \rar & \zxX{}      \rar & \zxZ{\pi} \rar &
    \end{ZX}, \quad 
    \begin{ZX}
        \rar &           \rar & \zxZ{} \dar \rar & \\
        \rar & \zxX{\pi} \rar & \zxX{}      \rar &
    \end{ZX} \overset{\zxref{f}}{=} 
    \begin{ZX}
        \rar & \zxZ{} \dar \rar &           \rar & \\
        \rar & \zxX{}      \rar & \zxX{\pi} \rar &
    \end{ZX}. \tag*\qedhere
\end{align}
\end{proof}

\begin{definition}
For a set $\cS \subseteq \cP_n$, define the \term{stabilizer subspace}\footnote{This is a strictly more restrictive definition of stabilizer subspaces and stabilizer groups than the definitions found in algebra, for instance. However, since we are only dealing with pure states, and are only concerned with the stabilizer subspaces of $\cP_n$, this definition suffices.} of $\cS$ as 
\begin{align}
    \Stab(\cS) := \{ \ket\psi \mid \vec{P} \ket\psi = \ket\psi, \forall \vec{P} \in \cS \}.
\end{align}
If $\Stab(\cS) \neq \{0\}$, call $\cS$ a \term{stabilizer group}.
\end{definition}

\begin{proof}
If $A$, $B$ are stabilizers of $\ket\psi$, then $AB \ket\psi = A \ket\psi = \ket\psi$ so $AB$ is also a stabilizer. Furthermore,
\begin{align}
    A \ket\psi = \ket \psi 
    \implies 
    \ket\psi = A^{-1} \ket\psi
\end{align}
so $A^{-1}$ is also a stabilizer of $\ket\psi$. Thus $\cS$ is a subgroup. On the other hand, if two states $\ket\psi$ and $\ket\phi$ are stabilized by $A$, then 
\begin{align}
    A (\lambda\ket\psi + \mu \ket\phi) 
    = \lambda A \ket\psi + \mu A \ket \phi 
    = \lambda \ket\psi + \mu \ket\phi,
\end{align}
for any $\lambda, \mu \in \C$. Since $0$ is trivially in every $\Stab(\cS)$, this is indeed a subspace.
\end{proof}

\begin{lemma}
\label{lemma:stab-group-properties}
If a subgroup $\cS \subseteq \cP_n$ is a stabilizer subgroup, then:
\begin{enumerate}
    \item $-I \notin \cS$,
    \item $\vec{P}^2 = I$ for all $\vec{P} \in \cS$, and
    \item $\cS$ is commutative.
\end{enumerate}
\end{lemma}

\begin{proof}
If $-I \in \cS$, then all $\ket\psi \in \Stab(\cS)$ satisfies $\ket\psi = -I \ket\psi = -\ket\psi$ such that $\Stab(\cS) = \{0\}$, a contradiction. An exhaustive multiplication table for $\cP_1$ reveals that if $\vec{P}^2 \neq I$, then $\vec{P}^2 = -I$ so again $\Stab(\cS) = \{0\}$. Thus, each $P \in \cS$ must have $P^2 = I$. Then $P^\dagger = P$, since $P^{-1} = P$ and its phase cannot be $\pm i$. Then for any $P,Q \in \cS$, $PQ = (PQ)^\dagger = Q^\dagger P^\dagger = QP$.
\end{proof}

\begin{theorem}
\label{thm:aaronson-sim-clifford}
This is Theorem 1 of \cite{sim_stab_circuits}. Given an $n$-qubit state $\ket\psi$, the following are equivalent:
\begin{enumerate}
    \item $\ket\psi$ can be obtained from $\ket{0}^{\otimes n}$ by CNOT, H, and S gates only.
    \item $\ket\psi$ can be obtained from $\ket{0}^{\otimes n}$ by CNOT, H, S, and measurement gates only.
    \item $\ket\psi$ is stabilized by exactly $2^n$ Pauli operators.
    \item $\ket\psi$ is uniquely determined by $\Stab(\ket\psi)$.
\end{enumerate}
Thus, we can all any circuit consisting entirely of Clifford gates and measurements a \term{stabilizer circuit}, and any state obtainable by applying a stabilizer circuit to $\ket{0}^{\otimes n}$ a \term{stabilizer state}.
\end{theorem}

\begin{proof}
A concise proof is available in \cite{sim_stab_circuits}, which itself relies on the Gottesman-Knill theorem (Theorem \ref{thm:Gottesman-Knill}), whose ZX-proof can be found in Chapter 5 of \cite{PQS} and non-ZX-proof in the original paper \cite{Gottesman-Knill}.
\end{proof}

\begin{lemma}
\label{lemma:size-of-pauli-stab}
Note that $|\cP_n| = 4^{n+1}$. Furthermore, the number of stabilizer states on $n$ qubits is:
\begin{align}
    2^n \prod_{i=0}^{n-1} (2^{n-i} + 1).
\end{align}
\end{lemma}

\begin{proof}
The first claim follows almost immediately from Definition \ref{def:pauli-group}. By Theorem \ref{thm:aaronson-sim-clifford}, to obtain the second claim we can equivalently count the number of ordered generating sets for stabilizer groups, divided by the number of generating sets for a given stabilizer. This is the proof used in Proposition 2 of \cite{sim_stab_circuits}.
\end{proof}

Informally, the stabilizer formalism tells us that the states obtainable by
applying Clifford circuits to $\ket{0}^{\otimes n}$ are exactly the pure
stabilizer states. This is useful because stabilizer states admit a compact
classical description. The tableau formalism below is one such description: it
keeps track of how a Clifford circuit conjugates a convenient set of Pauli
generators.

\begin{definition}
\label{def:tableau}
Let $\ket\psi$ be an $n$-qubit stabilizer state with stabilizer group
$\mathcal S(\ket\psi)$. A \term{tableau} for $\ket\psi$ is a binary matrix
encoding $2n$ signed Pauli strings
\begin{align}
    R_i = (-1)^{r_i} P_{i,1} \otimes \cdots \otimes P_{i,n},
    i \in [2n].
\end{align}
A tableau has the form
\begin{align}
\left[
\begin{array}{ccc|ccc|c}
x_{11} & \cdots & x_{1n}
&
z_{11} & \cdots & z_{1n}
&
r_1
\\
\vdots & \ddots & \vdots
&
\vdots & \ddots & \vdots
&
\vdots
\\
x_{n1} & \cdots & x_{nn}
&
z_{n1} & \cdots & z_{nn}
&
r_n
\\
\hline
x_{(n+1)1} & \cdots & x_{(n+1)n}
&
z_{(n+1)1} & \cdots & z_{(n+1)n}
&
r_{n+1}
\\
\vdots & \ddots & \vdots
&
\vdots & \ddots & \vdots
&
\vdots
\\
x_{(2n)1} & \cdots & x_{(2n)n}
&
z_{(2n)1} & \cdots & z_{(2n)n}
&
r_{2n}
\end{array}
\right],
\end{align}
where
\begin{align}
x_{ij}
&=
\begin{cases}
1 & \text{if } P_{i,j}=X \text{ or } P_{i,j}=Y, \\
0 & \text{otherwise,}
\end{cases}
&
z_{ij}
&=
\begin{cases}
1 & \text{if } P_{i,j}=Z \text{ or } P_{i,j}=Y, \\
0 & \text{otherwise.}
\end{cases}
\end{align}
The bit $r_i$ records the sign of the Pauli string:
\begin{align}
    r_i =
    \begin{cases}
    0 & \text{if } R_i \text{ has sign } +1,\\
    1 & \text{if } R_i \text{ has sign } -1.
    \end{cases}
\end{align}

The bottom $n$ rows are stabilizer generators for $\ket\psi$. The top $n$
rows are \term{destabilizer generators}. They do not stabilize $\ket\psi$.
Rather, they are auxiliary Pauli strings chosen so that, together with the
stabilizer generators, they form a convenient \emph{symplectic} basis of the Pauli
group. In the Aaronson--Gottesman tableau, the destabilizers are what make the
row operations and measurements efficient and canonical.
\end{definition}

\begin{theorem}
\label{thm:tableau-simulation}
Using the tableau representation, Clifford gates such as $H$, $S$, and
$\CNOT$ can be simulated efficiently by updating the binary entries of the
tableau. Pauli measurements can also be simulated efficiently using the same
representation. This gives the tableau-based form of the Gottesman--Knill
simulation algorithm \cite{sim_stab_circuits}.
\end{theorem}

\begin{proof}[Proof intuition]
The key observation is that each $P_{i,j}$ may be encoded by
two bits:
\begin{align}
    \begin{array}{c|cc}
    P & x & z \\
    \hline
    I & 0 & 0 \\
    X & 1 & 0 \\
    Z & 0 & 1 \\
    Y & 1 & 1
    \end{array}
\end{align}
Thus an $n$-qubit Pauli string can be encoded by $2n$ bits, together with one
additional sign bit. This is possible since stabilizer generators never have phases $\pm i$ by Lemma \ref{lemma:stab-group-properties}.

A Clifford gate maps Pauli strings to Pauli strings under conjugation. Hence,
to update the state after applying a Clifford gate, it suffices to update how
that gate conjugates each row of the tableau. For example, applying $H$ to
qubit $j$ swaps the $x_{ij}$ and $z_{ij}$ entries in column $j$, up to the
appropriate sign change when $Y$ is involved. Similarly, $S$ and $\CNOT$ act by
simple binary updates on the entries. Thus the tableau is
best viewed not as a list of stabilizers, but as an algorithmic data
structure for simulating stabilizer computations.
\end{proof}

Note that this algorithm may be improved further. See \cite{faster-sim-stab} for such an example.